...
def readfile(filename):
    ...
    for line in f:
        ...
        tag.append(splits[1])
        polarity.append(int(splits[2].strip()))
...
def convert(filename):
    ...
    for i, (word, aspect, polarity) in enumerate(data):
        raw_text = ""
        asp = ""
        sa = 0
        check = False
        for j, pol in enumerate(polarity):
            if pol != -1 and check is False:
                asp += str(word[j]) + " "
                raw_text += str("$T$") + " "
                sa = pol - 1
                check = True
            elif pol != -1 and check is True:
                continue
            else:
                raw_text += str(word[j]) + " "
                check = False
...
class ABSADataset(Dataset):
    def __init__(self, fname, tokenizer):
        ...
        for i in data:
                fileout.write(i)
        fileout.close()
        fin = open("output.txt", 'r', encoding='utf-8', newline='\n', errors='ignore')
        lines = fin.readlines()
        fin.close()
        all_data = []
        lines = lines[:-2]
