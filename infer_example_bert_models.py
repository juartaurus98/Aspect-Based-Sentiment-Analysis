...
from models.data_utils import Tokenizer4Bert
import argparse
...
if __name__ == '__main__':
    ...
    state_dict_paths = {
        'lcf_bert': 'output/absa/final_absa.pt',
    }
    ...
    input_text="မြန်မာမင်း များ ၏ ရာဇပလ္လင်ဥကင် လက်ဝဲ ၊ လက်ယာ တွင် ကိန္နရာ က နေ သည့် အသွင် ထုလုပ် ကြ သည် ။"

    model_ner = Ner("output/ner/")
    aspects = model_ner.predict(input_text)
    for aspect in aspects:
        text_left, asp, text_right = [s.lower().strip() for s in input_text.partition(aspect)]
        text_bert_indices, bert_segments_ids, text_raw_bert_indices, aspect_bert_indices = \
            prepare_data(text_left, asp, text_right, tokenizer)
